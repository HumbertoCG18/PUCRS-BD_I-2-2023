...
CREATE TABLE pedidos (  
    ...
    CONSTRAINT fk_peca FOREIGN KEY (PecaID) REFERENCES pecas(PecaID)  
);
;
...
FOR EACH ROW   
DECLARE  
    PrecoPeca BINARY_DOUBLE;  
BEGIN   
    SELECT Preco INTO PrecoPeca FROM pecas WHERE PecaID = :NEW.PecaID;   
    :NEW.PrecoUnitario := PrecoPeca;   
    :NEW.Total := :NEW.Quantidade * PrecoPeca;   
END; 
...
IS 
    v_estoque_disponivel NUMBER; 
BEGIN 

    SELECT Estoque 
    INTO v_estoque_disponivel 
    FROM pecas 
    WHERE PecaID = p_peca_id; 
...
ALTER TABLE log_pedidos ADD (Estoque NUMBER);
;
...
BEFORE INSERT ON log_pedidos     
FOR EACH ROW     
DECLARE     
    v_pode_registrar NUMBER;
BEGIN     
    SELECT Estoque INTO v_pode_registrar FROM log_pedidos WHERE peca_id = :NEW.peca_id;     
      
    IF v_pode_registrar >= :NEW.quantidade THEN     
        :NEW.pedido_id := SEQ_PEDIDO.NEXTVAL;     
